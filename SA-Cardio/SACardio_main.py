...
DATASET_PATH = r"C:\Users\aralmeida\OneDrive - Universidad de Las Palmas de Gran Canaria\Doctorado\Bases de datos\Cardiovascular\SouthAfrica"
...
data, X, Y, feat_names, y_tag = prepare_SACardio(DATASET_PATH, filename)

# Return from dataset directory to working directory 
os.chdir(wd)

# Exploratory Data Analysis
eda(data, X, Y, 'SA-Cardio')
...
eda(train_data, X_train, y_train, 'SACardio', folder = r"./EDA_train")
eda(validation_data, X_val, y_val, 'SACardio', folder = r"./EDA_val")
...
# Defining metadata for SA-Cardiovascular database
metadata = Metadata()
metadata.add_table(
    name = 'SA-Cardio',
    data = train_data)
    #fields_metadata = SACardio_fields)

# A) Gaussian Copula model
gc = GaussianCopula(field_types = SACardio_fields, 
                    #constraints = constraints, 
                    field_distributions = SACardio_distributions)
 
# B) CTGAN model 
ctgan = CTGAN(field_types = SACardio_fields,
              cuda = True)
...
print("Computation time expended by this framework with SACardio database: %f" % total_time)
...
plt.savefig('SA-Cardio_metrics_vs_synthetic_data_samples', dpi=600)
...
plt.savefig('SA-Cardio_f1_vs_data_samples', dpi = 600)
...
ax[0].legend(models, bbox_to_anchor=(0.07,1.02,1,0.2), loc="lower left",
                mode="None", borderaxespad=0, ncol=4, prop={'size': 6})
# Save figure
plt.savefig('SA-Cardio_sdg_metrics_vs_f1_score', dpi=600)