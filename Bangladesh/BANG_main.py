...
####################################      ARGUMENTS       ##############################################
# Dataset path
DATASET_PATH = r"C:\Users\aralmeida\OneDrive - Universidad de Las Palmas de Gran Canaria\Doctorado\Bases de datos\Diabetes\Bangladesh"

# File name 
filename = "Dataset Diabetes Type1 (Total).csv"
...
bal_methods = ["ADASYN", "SMOTE", "SMOTENC", "KMeansSMOTE", "SVMSMOTE", "BorderlineSMOTE"]
...
cat_feat_idxs = cat_feat_idxs = [1,2,3,4,12,13]

# Differentiating between categorical and numerical features. The former are 
# one-hot encoded and the latter are standardized
categorical_features = ['Age','Sex','Area of Residence','HbA1c','Standardized growth-rate',
                        'Standardized birth weight']
numerical_features = ['Height','Weight','BMI','Duration of disease']
...
data, X, Y, feat_names, y_tag = prepare_BANG(dataset_path = DATASET_PATH, filename = filename)

# Return from dataset directory to working directory 
os.chdir(wd)

# Exploratory Data Analysis
#eda(data, X, Y, 'ALZ-Balea')
...
# Fix fields to int fields (related to SDV library bug at the current version, see requirements.txt)
train_data['pancreatic disease affected in child '] = train_data['pancreatic disease affected in child '].replace([1,0],[1,0])
data_balance1['pancreatic disease affected in child '] = data_balance1['pancreatic disease affected in child '].replace([1,0],[1,0])
data_balance2['pancreatic disease affected in child '] = data_balance2['pancreatic disease affected in child '].replace([1,0],[1,0])
train_data['Autoantibodies'] = train_data['Autoantibodies'].replace([1,0],[1,0])
data_balance1['Autoantibodies'] = data_balance1['Autoantibodies'].replace([1,0],[1,0])
data_balance2['Autoantibodies'] = data_balance2['Autoantibodies'].replace([1,0],[1,0])
...
metadata.add_table(
    name = dataset_name,
    data = train_data,
    fields_metadata = bang_fields)

# A) Gaussian Copula model 
gc = GaussianCopula(field_types = bang_fields, 
                    #constraints = constraints, 
                    field_distributions = bang_distributions)

 # B) CTGAN model 
ctgan = CTGAN(field_types = bang_fields,
            constraints=constraints,
            cuda = True)
...
best_worst = ['Borderline + Sep. + GC', 'Borderline + CTGAN'] 
