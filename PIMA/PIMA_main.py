...
bal_methods = ["ADASYN", "SMOTE", None, "KMeansSMOTE", "SVMSMOTE", "BorderlineSMOTE"]
...
if BALANCING_CHECKED  == False : 
    ...
    balancing_eval(dataset_name, X_train, y_train, train_data,
                 ...
                 None ,filename = "balancing_metrics.csv" , iterations = bal_iterations, store_path = STORE_PATH) 
...
# B) CTGAN model 
ctgan = CTGAN(field_types = pima_fields, 
            #constraints=constraints, 
            cuda = True)
...
sizes_balance2 = [round(data_balance2.shape[0]/4), round(data_balance2.shape[0]/2), round(data_balance2.shape[0]), 
         round(data_balance2.shape[0]*2), round(data_balance2.shape[0]*4), round(data_balance1.shape[0]*4)+20]
sizes_only_augmen1 = [round(X_train.shape[0]/4), round(X_train.shape[0]/2), round(X_train.shape[0]), 
         round(X_train.shape[0]*2), round(X_train.shape[0]*4), round(data_balance1.shape[0]*4)+20]
...
#print("Computation time expended by this framework with %s database: %f" % (dataset_name, total_time))
with open("time.txt", "wb") as t:
