...
if BALANCING_CHECKED  == False : 
    ...
    balancing_eval('PIMA', X_train, y_train, train_data,
                 feat_names, y_tag, ["ADASYN", "SMOTE", None, "KMeansSMOTE", "SVMSMOTE", "BorderlineSMOTE"], 
                 None ,filename = "balancing_metrics.csv" , iterations = iterations, store_path = STORE_PATH) 
...
diabetes_balance1 = data_balance1.loc[(data_balance1[y_tag]==1)]
# Borderline-SMOTE
controls_balance2 = data_balance2.loc[(data_balance2[y_tag]==0)] 
diabetes_balance2 = data_balance2.loc[(data_balance2[y_tag]==1)]

# Defining metadata for PIMA database
metadata = Metadata()
metadata.add_table(
    name = 'PIMA',
    data = X,
    fields_metadata = pima_fields)

# A) Gaussian Copula model 
gc = GaussianCopula(field_types = pima_fields, 
                    constraints = constraints, 
                    field_distributions = pima_distributions)
# B) CTGAN model 
ctgan = CTGAN(field_types = pima_fields, constraints=constraints, cuda = True)
...
split = [(augmen1, balance1, controls_balance1, diabetes_balance1, sizes_balance1),
(augmen1, balance2, controls_balance2, diabetes_balance2, sizes_balance2),
(augmen2, balance1, controls_balance1, diabetes_balance1, sizes_balance1),
(augmen2, balance2, controls_balance2, diabetes_balance2, sizes_balance2)]
...
X_train_norm, y_train_norm = standardization(X_train, y_train)
X_val_norm, y_val_norm = standardization(X_val, y_val)
...
for i in range(len(sizes_keys)):
    ...
    for j in range(iterations):
        ...
        for group in no_split : 
            ...
            KLD_val, _, _ = KLD(mixed_data[feat_names], train_PMFs, train_hist_bases)
            MMD_val = mmd_linear(X_train.to_numpy(), mixed_data[feat_names].to_numpy())
            ...
            # Standardization
            X_norm, y_norm = standardization(mixed_data[feat_names], mixed_data[y_tag])
...
print("Computation time expended by this framework with PIMA database: %f" % total_time)
with open("PIMA_time", "wb") as t:
    pickle.dump(total_time, t)
t.close()
...
plt.savefig('PIMA_metrics_vs_synthetic_data_samples', dpi=600)
...
plt.savefig('PIMA_f1_vs_data_samples', dpi = 600)
...
ax[0].legend(models, bbox_to_anchor=(0.07,1.02,1,0.2), loc="lower left",
                mode="None", borderaxespad=0, ncol=4, prop={'size': 6})
# Save figure
plt.savefig('PIMA_sdg_metrics_vs_f1_score', dpi=600)