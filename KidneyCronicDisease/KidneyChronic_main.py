...
DATASET_PATH = r"C:\Users\aralmeida\OneDrive - Universidad de Las Palmas de Gran Canaria\Doctorado\Bases de datos\Chronic Kidney Disease - UCI\Chronic_Kidney_Disease"
...
data, X, Y, feat_names, y_tag = prepare_KidneyChronic(DATASET_PATH, filename)
...
gc = GaussianCopula(field_types = kidneyChronic_fields, 
                    #constraints = constraints, 
                    field_distributions = kidneyChronic_distributions)
 
# B) CTGAN model 
ctgan = CTGAN(field_types = kidneyChronic_fields,
              cuda = True)
...
categorical_features = ['sg', 'al', 'su', 'rbc', 'pc', 'appet']
numerical_features = ['age', 'bp', 'bgr', 'sc', 'sod', 'pot', 'hemo', 'pcv', 'wc', 'rc']
...
plt.savefig('KidneyChronic_metrics_vs_synthetic_data_samples', dpi=600)
...
plt.savefig('KidneyChronic_f1_vs_data_samples', dpi = 600)
...
ax[0].legend(models, bbox_to_anchor=(0.07,1.02,1,0.2), loc="lower left",
                mode="None", borderaxespad=0, ncol=4, prop={'size': 6})
# Save figure
plt.savefig('KidneyChronic_sdg_metrics_vs_f1_score', dpi=600)