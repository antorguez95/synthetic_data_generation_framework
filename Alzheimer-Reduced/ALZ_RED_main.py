...
# File name 
filename1 = "Grupo1.xlsx"
filename2 = "Grupo2.xlsx"
...
categorical_features = ['Convivencia']
...
data, X, Y, feat_names, y_tag = prepare_ALZ_RED(dataset_path = DATASET_PATH, filename1 = filename1, filename2 = filename2)
...
metadata.add_table(
    name = dataset_name,
    data = train_data,
    fields_metadata = alz_red_fields)

# A) Gaussian Copula model 
gc = GaussianCopula(field_types = alz_red_fields, 
                    #constraints = constraints, 
                    field_distributions = alz_red_distributions)
 
# B) CTGAN model 
ctgan = CTGAN(field_types = alz_red_fields,
              constraints=constraints, 
              cuda = True)
